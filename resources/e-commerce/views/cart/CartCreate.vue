      ...
      <div v-for="(item, index) in cart.items" :key="index" class="d-sm-flex justify-content-between align-items-center my-2 pb-3 border-bottom">
        <div class="d-block d-sm-flex align-items-center text-center text-sm-start">
          ...
          <div class="pt-2">
            ...
            <div v-if="item.variantId" class="fs-sm mb-1"><span class="text-muted me-2">Variant: {{ item.variantId }}</span></div>
            <div v-else-if="item.color" class="fs-sm mb-1"><span class="text-muted me-2">Color: {{ item.color }}</span></div>
