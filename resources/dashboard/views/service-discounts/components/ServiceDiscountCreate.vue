...
methods: {
	...mapActions('ServiceDiscounts', ['fetchOne','persist', 'setProperty', 'attachRelations','detachRelations', 'resetSelected']),
	fetchItems() {
		this.setProperty({property: 'relations', value:['product']});
		this.fetchOne({id: this.id}).then(() => {
			this.selectedProduct = this.selected.product
		});
	},
	addProduct() {
	...
	handleAddProducts() {
		if(parseFloat(this.selected.discount_amount) > parseFloat(this.selectedProduct.commission)) {
			this.$toast.error('That discount amount is invalid');
			this.$toast.info('Consider asking the service provider for discount on that service');
			return;
		}
		this.persist().then(({data}) => {
			this.$toast.success('Item added Successfully');
			setTimeout(() =>{
				window.location = "/service-discounts";
			},200)
		}).catch(({response}) => {
	...
	<div class="card-body">
		<div class="row">
			<div class="col-6 offset-2 text-center">
				<div class="mb-2 alart alert-primary"><p class="p-3">Select Service to Discount</p></div>
				<button type="button" class="btn btn-primary" @click="addProduct">Select Service</button>
			</div>
		</div>
					...
					<div class="row">
						<div class="col-6">
							...
							<div class="form-group">
								...
								<input class="form-control" :value="selected.discount_amount" type="number" :max="selectedProduct.commission"  min="1" @input="(e) => updateProperty('discount_amount', e.target.value)">
							</div>
						...
						<div class="col-6">
							...
							<div class="form-group">
								...
								:disabled="parseFloat(rembekaCommission) < 0"
								>{{ selected.id ? 'Update':'Add'}} discount</button>
							</div>
						</div>
					</div>
				</div>
  ...
  <div class="modal-dialog modal-fullscreen">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="selecteProductModal">Add Service</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
				<services-index  @selected="(item) => itemChecked(item)" :show-actions="false" selectable/>
      </div>
      <div class="modal-footer" v-if="selectedProduct">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        <button type="button" class="btn btn-primary" @click="selectProduct">Select Service</button>
      </div>
    </div>
  </div>
</div>
