        ...
        @if($bestseller->product->attachments && $bestseller->product->attachments->count() > 0)
          <div class="card product-card card-static pb-3">
            ...
            <a class="card-img-top d-block overflow-hidden text-center"
              href="{{ route('product.show', ['slug' => $bestseller->product->slug, 'productId' => $bestseller->product->id])}}">
              <img class="product-image"
                src="{{ optional(optional($bestseller->product->attachments->first())->media)->url }}" alt="Product" />
            </a>
            <div class="card-body py-2">
              ...
              <h3 class="product-title fs-sm text-truncate"><a
                  href="{{route('product.show', ['slug' => $bestseller->product->slug, 'productId' => $bestseller->product->id])}}">{{$bestseller->product->name }}</a>
              </h3>
              <div class="product-price"><span class="text-accent"> Ksh {{$bestseller->product->final_price}} </div>
            </div>
            <div class="product-floating-btn">
              <a href="{{ route('product.show', ['slug' => $bestseller->product->slug, 'productId' => $bestseller->product->id])}}"
